...
const OrderRestore = () => {
    ...
    const getOders = async (page) => {
        try {
            const response = await axios.get(`http://localhost:5000/api/admin-order/track-order/track?page=${page}`)
    ...
    const deleteProduct = async productId => {
        try {
            const response = await axios.delete(`http://localhost:5000/api/admin-order/${productId}`)
            if (response.data.success) {
    ...
    const deletProductById = async productId => {
        const confirmBox = window.confirm(
            "Bạn có chắc chắn muốn xoá vĩnh viễn hoá đơn?"
        )
        if (confirmBox === true) {
    ...
    const retoreProductById = async productId => {
        const confirmBox = window.confirm(
            "Bạn có chắc chắn muốn khôi phục hoá đơn?"
        )
        if (confirmBox === true) {
                            ...
                            <Grid container alignItems="center">
                                ...
                                >
                                    <Button
                                        ...
                                        onClick={retoreProductById.bind(this, order._id)}
                ...
                <Grid item md={12} xs={12}>
                    ...
                    <Card className="p-0">
                        <div className="mb-1 flex justify-between items-center">
                            <h4 className="font-medium">Khôi phục hoá đơn</h4>
                        </div>
                        ...
                        <div className="flex mb-5 justify-between items-center h-full">
                            <div className="flex">
                                ...
                                />
                                <Button variant='secondary' className="image8" to={'/orders/manage'} as={Link}>
                                    Quay lại
                                </Button>
                            </div>
