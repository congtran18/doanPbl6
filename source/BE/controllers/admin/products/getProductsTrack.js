...
async function getProductsTrack(req, res, next) {
    ...
    query = await Product.find({...productfilter, ... {track : false}}).sort({createdAt:-1}).populate('type').populate('category').skip(skip).limit(pageSize);

    total = await Product.countDocuments({...productfilter, ... {track : false}});
...
module.exports = getProductsTrack;
